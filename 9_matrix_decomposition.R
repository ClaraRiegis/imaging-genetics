...
# From Varun Warrier's GitHub
# https://github.com/vwarrier/Imaging_genetics_analyses/blob/main/Matrix_decomposition.R

setwd("~/Desktop/Cambridge/mphil_project")  
dir_path <- 'Figures/correlations/'
files = c('genetic', 'pheno')

for (file in files){
  print(file)
  merged <- read.csv(paste(dir_path, file, '_corr_mat.csv', sep = ''), header = TRUE)
                 
  correlation.matrix <- merged #as.data.frame(cor(merged, method = "pearson", use="pairwise.complete.obs"))
  
  #save(correlation.matrix, file = "ABCD/abcd_format_data/correlationbetweenROIS.rds")
  ...
  sink(paste(dir_path, '/mat_decomp_', file,".txt", sep = ''))
